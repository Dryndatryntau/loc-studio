...
// ==========================================================================================
// 2. IŠMANI FUNKCIJA: Grąžina failo info tiesiai iš RAM ir griežtai sukalibruoja tipus!
// ==========================================================================================
extern "C" __declspec(dllexport) bool GetH4RFileInfo(const char* h4rPath, int index, char* outName, unsigned int* outSize, unsigned int* outCompSize, unsigned int* outType)
{
    ...
    // A. Nukopijuojame originalų pavadinimą į Embarcadero buferį
    strcpy(outName, globalArchive.m_ppName[index]);

    // B. Perduodame realius binarinius dydžius
    *outSize = globalArchive.m_pUnpSize[index];
    *outCompSize = globalArchive.m_pSize[index];

    // C. IŠMANUSIS TIPO ATPAŽINIMAS PAGAL TEKSTINIUS PREFIKSUS (Kalibracija pagal tavo TComboBox!)
    // Pagal nutylėjimą nustatome paskutinį tipą - 20 (Others)
    unsigned int calculatedType = 20;

    const char* namePtr = globalArchive.m_ppName[index];

    if (strncmp(namePtr, "table.", 6) == 0) {
        // Tikriname ar tai nėra specifinė kovos lentelė
    ...
    // Atiduodame tikslų ir griežtai sukalibruotą indeksą į C++Builder programą!
            ...
            if (destFile != NULL) {
                ...
                if (compressedSize < uncompressedSize) {
                    ...
                    if (uncompressedBuffer != NULL) {
                        ...
                        // NAUDOJAME ŽEMO LYGIO ZLIB STRUKTŪRĄ (Paimta iš pačio AKuHAK dump funkcijos eilučių!)
                        z_stream zStream;
                        ...
                        // Inicializuojame gzip/zlib dekompresiją fone
                        if (inflateInit2(&zStream, 15 + 32) == Z_OK) {
                            ...
                            if (inflateResult == Z_STREAM_END || inflateResult == Z_OK) {
                                // Įrašome ŠVARŲ, dekompresuotą lietuvišką tekstą!
